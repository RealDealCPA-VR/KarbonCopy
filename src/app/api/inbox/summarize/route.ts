...
import { aiEnabled, summarizeThread } from "@/lib/ai";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[inbox/summarize] buildThreadText failed:", (err as Error).message);
    return NextResponse.json({ error: "Failed to load thread." }, { status: 500 });
  }
  if (!text) return NextResponse.json({ error: "Thread not found." }, { status: 404 });

  try {
    const summary = await summarizeThread(text);
